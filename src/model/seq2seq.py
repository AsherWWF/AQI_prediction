...
class Decoder(Block):
    ...
    def sampling(self):
        ''' Wether sampling the ground truth. '''
...
class Seq2Seq(Block):
    ...
    def forward(self, feature, data, label, mask, is_training):
        ...
        Args:
            ...
            mask: NDArray with shape [b, t, n, d].
            is_training: boolean.

        Returns:
            loss: loss for gradient descent.
            (pred, label, mask): aach of them is a NDArray with shape [n, b, t, d].
        """
        ...
        # geo-feature embedding
        feature = self.geo_encoder(nd.mean(feature, axis=0)) # shape=[n, d]

        # PCA TEST
        # from sklearn.decomposition import PCA
        # pca = PCA(n_components=2)
        # pca.fit(feature.asnumpy())
        # print(pca.transform(feature.asnumpy()[-20:]))
        # exit()
