...
from model.basic_structure import HyperDense
...
class RNNCell(Block):
    ...
    def create(rnn_type, pre_hidden_size, hidden_size, prefix):
        ...
        elif rnn_type == 'HyperGRUCell': return HyperGRUCell(pre_hidden_size, hidden_size, hyper_hiddens=[16,2], prefix=prefix)
        elif rnn_type == 'EmbeddingCell': return EmbeddingCell(hidden_size, prefix)
...
class EmbeddingCell(RNNCell):
    def __init__(self, hidden_size, prefix=None):
        super(EmbeddingCell, self).__init__(prefix=prefix)
        self.hidden_size = hidden_size
        with self.name_scope():
            self.cell = nn.Dense(hidden_size, activation='relu', flatten=False)
    
    def forward_single(self, feature, data, begin_state):
        return self.cell(data), None
...
class HyperGRUCell(RNNCell):
    def __init__(self, pre_hidden_size, hidden_size, hyper_hiddens, prefix=None):
        super(HyperGRUCell, self).__init__(prefix=prefix)
        self.pre_hidden_size = pre_hidden_size
        self.hidden_size = hidden_size
        with self.name_scope():
            self.dense_z = HyperDense(pre_hidden_size + hidden_size, hidden_size, hyper_hiddens=hyper_hiddens, prefix='dense_z_')
            self.dense_r = HyperDense(pre_hidden_size + hidden_size, hidden_size, hyper_hiddens=hyper_hiddens, prefix='dense_r_')

            self.dense_i2h = HyperDense(pre_hidden_size, hidden_size, hyper_hiddens=hyper_hiddens, prefix='dense_i2h_')
            self.dense_h2h = HyperDense(hidden_size, hidden_size, hyper_hiddens=hyper_hiddens, prefix='dense_h2h_')
