	...
	def process_data(self, epoch, dataloader, metrics=None, is_training=True, title='[TRAIN]'):
		...
		for nbatch, batch_data in enumerate(dataloader):
			...
			if is_training:
				...
				# print('w', self.net.collect_params()['seq2seq_decoder_c1_dense_z_w_dense2_weight'].grad(mx.gpu(0)).sum())
				# print('b', self.net.collect_params()['seq2seq_decoder_c1_dense_z_w_dense2_bias'].grad(mx.gpu(0)).sum())
